...
const ArtworksPage = ({ onNavigate }) => {
  const { data, loading, error, refetch } = useFetch(
    () => itemsAPI.getAll({ category: "artwork", status: "active", limit: 48 }),
    []
  );
      ...
      <main className="max-w-6xl mx-auto px-4 py-10">
        ...
        {loading ? (
          ...
          <div className="card p-16 text-center">
            <Palette size={48} className="mx-auto mb-4 text-soil-300" strokeWidth={1.5} />
            <h2 className="font-display font-bold text-soil-700 text-xl mb-2">No artworks yet</h2>
            <p className="text-soil-400 text-sm max-w-md mx-auto">
              Our artists are busy turning scrap into art. Check back soon or sign in to list your own.
            </p>
            <button onClick={() => onNavigate("landing")} className="btn-outline mt-6">
              Back to home
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {items.map((item) => {
              ...
              return (
                ...
                >
                  <div className={`h-36 flex items-center justify-center text-4xl overflow-hidden border-b ${cat.border} ${cat.bg}`}>
                    {item.images?.[0]?.url ? (
                      <img src={item.images[0].url} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    ) : (
